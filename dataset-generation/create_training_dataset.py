...
def extract_cards_from_video(video_fn, output_dir=None, keep_ratio=5, min_focus=50, debug=False):
    """
...
def extract_cards_from_all_videos():
...
def calculate_all_convex_hulls():
    imgs_dir = "data/cards"
    ...
    for suit in card_suits:
        for value in card_values:
            ...
            for f in glob(card_dir + "/*.png"):
                ...
                img = cv2.cvtColor(img, cv2.COLOR_BGRA2RGBA)
                cards_dic[card_name].append((img, hullHL, hullLR))
            print(f"Nb images for {card_name} : {len(cards_dic[card_name])}")
...
class Backgrounds:
    def __init__(self, backgrounds_pickle=backgrounds_pck_fn):
        self._images = pickle.load(open(backgrounds_pickle, 'rb'))
...
def generate_datasets():
    ...
    for _ in tqdm(range(nb_cards_to_generate)):
        background = backgrounds.get_random()
        img1, card_val1, img1_hull1, img1_hull2 = cards.get_random()
        img2, card_val2, img2_hull1, img2_hull2 = cards.get_random()

        new_img = Scene(background, img1, card_val1, img1_hull1, img1_hull2, img2, card_val2, img2_hull1, img2_hull2)
    ...
    for _ in tqdm(range(nb_cards_to_generate)):
        ...
        img1, card_val1, img1_hull1, img1_hull2 = cards.get_random()
        img2, card_val2, img2_hull1, img2_hull2 = cards.get_random()
        img3, card_val3, img3_hull1, img3_hull2 = cards.get_random()

        new_img = Scene(background, img1, card_val1, img1_hull1, img1_hull2, img2, card_val2, img2_hull1, img2_hull2,
                        img3, card_val3, img3_hull1,
                        img3_hull2)
        new_img.write_files(save_dir)
...
if __name__ == "__main__":
    load_dtd_images()
    extract_cards_from_all_videos()
    calculate_all_convex_hulls()
    generate_datasets()
